...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[nn.Module, torch.Tensor]:
        ...
        # modules = self.submodule._modules.items()
        modules = self.submodule
...
class SubtractiveResetIFNodes(nodes.Nodes):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.total_spikes = torch.tensor([0], dtype=torch.float32)
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        new_spikes = torch.tensor(
                [self.total_spikes[-1]+ self.s.sum().data.float().cpu()]
                )
        prev = self.total_spikes
        self.total_spikes = torch.cat(
            (prev,  new_spikes)
        )
...
# class ZeroResetIFNodes(nodes.Nodes):
#     # language=rst
#     """
#     Layer of `integrate-and-fire (IF) neurons <http://neuronaldynamics.epfl.ch/online/Ch1.S3.html>`_ using reset by
#     subtraction.
...
def data_based_normalization(
        ...
        for layer in network.children():
            ...
            if not list(layer.children()):  # if leaf node, add it to list
                if isinstance(layer, nn.Conv2d):
                    ...
                    all_layers['MaxPool.' + str(count['pooling'])] = layer
                    count['pooling'] += 1
    ...
    extractor = FeatureExtractor(all_layers)
    ...
    for name, module in all_layers.items():
        ...
        if isinstance(module, nn.Sequential):
            ...
            def get_count(a, b, x):
                return np.sum((x.numpy()>a)&(x.numpy()<b))
...
def _ann_to_snn_helper(prev, current, node_type, nxt, last=False, **kwargs):
    # language=rst
    """
    ...
    if isinstance(current, nn.Linear):
        ...
        elif isinstance(nxt, nn.AvgPool2d) or isinstance(nxt, nn.MaxPool2d):
            shape = (out_channels, int(width), int(height))
        ...
        input_height, input_width = prev.shape[-2], prev.shape[-1]
    ...
    elif isinstance(current, nn.AvgPool2d):
        input_height, input_width = prev.shape[-2], prev.shape[-1]
        current.kernel_size = _pair(current.kernel_size)
        current.padding = _pair(current.padding)
        current.stride = _pair(current.stride)
...
def ann_to_snn(
    ...
    **kwargs,
    ...
    for c in ann.children():
        if isinstance(c, nn.DataParallel):
            c = list(c.children())[0]
    ...
    while i < len(children) - 1:
        ...
        layer, connection = _ann_to_snn_helper(prev, current, node_type, nxt, **kwargs)
    ...
    layer, connection = _ann_to_snn_helper(
        prev, current, node_type, nxt=None, last=True, **kwargs
    )
